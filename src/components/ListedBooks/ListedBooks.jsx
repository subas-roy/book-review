    ...
    <div>
      ...
      <div role="tablist" className="tabs tabs-lifted">
        ...
        <div role="tabpanel" className="tab-content bg-base-100 border-base-300 rounded-sm p-6">
          {
            readbooks.length === 0 ? (
              <p>No books are available in the read list.</p>
              ) : (
                (displayReadBooks.length === 0 ? readbooks : displayReadBooks).map(readItem => 
                <ListItem key={readItem.id} readItem={readItem}></ListItem>
              )
            )
          }
        </div>
        ...
        <div role="tabpanel" className="tab-content bg-base-100 border-base-300 rounded-sm p-6">
          {
            wishbooks.length === 0 ? (
              <p>No books are available in the wishlist.</p>
              ) : (
                (displayWishBooks.length === 0 ? wishbooks : displayWishBooks).map(readItem => 
                <ListItem key={readItem.id} readItem={readItem}></ListItem>
              )
            )
          }
